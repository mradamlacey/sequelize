  ...
  var queryAndEmit = QueryInterface.prototype.queryAndEmit = function(sqlOrQueryParams, methodName, options, emitter) {
    ...
    if (!!emitter) {
      execQuery(emitter)
       ...
       var customEmitter = new Utils.CustomEventEmitter(execQuery);

       if(this.sequelize.options.continuationLocalStorageEnabled){
         this.sequelize.options.continuationLocationStorageNamespace.bindEmitter(customEmitter);
       }
