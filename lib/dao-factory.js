...
module.exports = (function() {
 ...
 ;(function() {
  ...
  DAOFactory.prototype.init = function(daoFactoryManager) {
    ...
    this.continuationLocalStorageEnabled      = this.daoFactoryManager.sequelize.options.continuationLocalStorageEnabled
    this.continuationLocationStorageNamespace = this.daoFactoryManager.sequelize.options.continuationLocationStorageNamespace
  ...
  DAOFactory.prototype.sync = function(options) {
    ...
    var customEventEmitter = new Utils.CustomEventEmitter(function(emitter) {
    ...
    if(self.continuationLocalStorageEnabled){
        self.continuationLocationStorageNamespace.bindEmitter(customEventEmitter);
    }
  ...
  */
  DAOFactory.prototype.find = function(options, queryOptions) {
    var hasJoin = false
      ...
      //console.log('[sequelize] - find - current user: ' + this.daoFactoryManager.sequelize.options.continuationLocationStorageNamespace.get('currentUser'));

    // no options defined?
    // return an emitter which emits null
    if ([null, undefined].indexOf(options) !== -1) {
      var customEmitter = new Utils.CustomEventEmitter(function(emitter) {

        //  console.log('[sequelize] - find - emitter 1 current user: ' + this.daoFactoryManager.sequelize.options.continuationLocationStorageNamespace.get('currentUser'));

        setTimeout(function() { emitter.emit('success', null) }, 10)
      });
      if(this.daoFactoryManager.sequelize.options.continuationLocalStorageEnabled){
        this.daoFactoryManager.sequelize.options.continuationLocationStorageNamespace.bindEmitter(customEmitter);
      }

      return customEmitter.run();
    }
  ...
  DAOFactory.prototype.update = function(attrValueHash, where, options) {
    ...
    return new Utils.CustomEventEmitter(function(emitter) {
        //console.log('in update event emitter: ' + self.options.namespace);
