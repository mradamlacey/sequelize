  ...
  */
  DAOFactory.prototype.find = function(options, queryOptions) {
    var hasJoin = false
      ...
      console.log('[sequelize] - find - current user: ' + this.daoFactoryManager.sequelize.options.namespace.get('currentUser'));
    ...
    if ([null, undefined].indexOf(options) !== -1) {
      var customEmitter = new Utils.CustomEventEmitter(function(emitter) {
          ...
          console.log('[sequelize] - find - emitter 1 current user: ' + this.daoFactoryManager.sequelize.options.namespace.get('currentUser'));

        setTimeout(function() { emitter.emit('success', null) }, 10)
      });

      this.daoFactoryManager.sequelize.options.namespace.bindEmitter(customEmitter);
