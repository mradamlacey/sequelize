...
module.exports = (function() {
  var DAO = function(values, options) {
    ...
    this.__sequelizeOptions          = this.__factory.daoFactoryManager.sequelize.options
  ...
  DAO.prototype.save = function(fieldsOrOptions, options) {
    ...
    if (options.fields) {
      ...
      //console.log('CLS namespace: ' + self.__sequelizeOptions.namespace);

    var customEmitter =  new Utils.CustomEventEmitter(function(emitter) {

        //console.log('[sequelize] - current user: ' + self.__sequelizeOptions.namespace.get('currentUser'));

      self.hookValidate().error(function(err) {
        emitter.emit('error', err)
      }).success(function() {
        //  console.log('[sequelize] - success - current user: ' + self.__sequelizeOptions.namespace.get('currentUser'));
    ...
    if(self.__sequelizeOptions.continuationLocalStorageEnabled){
      self.__sequelizeOptions.continuationLocationStorageNamespace.bindEmitter(customEmitter);
    }
